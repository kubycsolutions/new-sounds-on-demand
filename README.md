...
***Unfortunately, development has been shelved as of 2025. I never did find time to update to Jovo Version 4, and the Jovo project has been shut down and archived. The published Alexa skill remains live, but currently has known issues updating the episode database from the New Sounds archive so episodes after number 4000 or so are not available. It is unclear whether I will be able to revive it using this code, or if I will have to reimplement using another set of UI calls.***
...
There have been over 4500 unique episodes since the early 1980s, with new ones still being added, and most of them can now be retrieved from the station's servers. This skill lets Alexa-family smart speakers access those archives, selecting them by broadcast date or episode number or asking for oldest/newest/random. We can also tune in the New Sounds live stream, a 24-hour mix with the same wide variety.
