...
// Need to think about whether to start loading/searching multiple
// shows to include Soundcheck, since it's the other "recorded live"
// part of NewSounds... which will require parameterizing the
// station-database URI since I don't think there's an all-shows query
// thereupon. (Though their API *is* mostly undocumented, so it may
// have more flexibility than I've been able to access. Ideal, of
// course, would be to be able to run the smartspeaker app directly
// against their data, but the part of the API I've figured out isn't
// flexibile enough for more than paged access.)

const table=process.env.NSOD_EPISODES_TABLE || "episodes_debug"
const program=process.env.NSOD_PROGRAM || "newsounds"

createAndLoad(-1) // Incremental
